...
class BusinessInformation extends DBModel
{
    ...
    public function images_our_products_preview()
    {
        return $this->hasMany(ImageResized::class, 'entity_id')->where('grid', 'preview')->where('content', 'ourproducts')->where('entity', 'business_information');
    }

    public function image_our_products_desktop()
    {
        return $this->hasOne(ImageResized::class, 'entity_id')->where('grid', 'desktop')->where('content', 'ourproducts')->where('entity', 'business_information')->where('language', App::getLocale());
    }

    public function image_our_products_mobile()
    {
        return $this->hasOne(ImageResized::class, 'entity_id')->where('grid', 'mobile')->where('content', 'ourproducts')->where('entity', 'business_information')->where('language', App::getLocale());
    }
}
